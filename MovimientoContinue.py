...
while 1:
    ...
    if keys[pg.K_a] and dino_rect.left > 0:
        dino_rect.x -= SPEED
    if keys[pg.K_d] and dino_rect.right < WIDTH:
        dino_rect.x += SPEED
    if keys[pg.K_w] and dino_rect.top > 0:
        dino_rect.y -= SPEED
    if keys[pg.K_s] and dino_rect.bottom < HEIGHT:
        dino_rect.y += SPEED
